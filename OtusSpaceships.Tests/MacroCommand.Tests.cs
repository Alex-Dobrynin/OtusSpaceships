...
namespace OtusSpaceships.Tests
{
    public class MacroCommandTests
    {
        ...
        public void RotateAlgo_ShouldRotateAndCorrelateVelocity(int startVelocityX, int startVelocityY, int startDirection, int angularVelocity, int endVelocityX, int endVelocityY, int endDirection)
        {
            ...
            _velocityChangableMock.SetupGet(r => r.Direction).Returns(endDirection);
            _velocityChangableMock.SetupGet(r => r.AmountOfDirections).Returns(AmountOfDirections);
            _velocityChangableMock.SetupProperty(x => x.Velocity, new Vector(startVelocityX, startVelocityY));
            var rotate = new RotateCommand(_rotatableMock.Object);
            var correlate = new CorrelateVelocityCommand(_velocityChangableMock.Object);
            var macro = new MacroCommand(rotate, correlate);

            macro.Execute();

            _rotatableMock.Object.Direction.Should().Be(endDirection);
            _velocityChangableMock.Object.Velocity.Should().Be(new Vector(endVelocityX, endVelocityY));
        }
    }
}
